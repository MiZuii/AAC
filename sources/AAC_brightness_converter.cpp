...
AAC_BC_Simple::AAC_BC_Simple(float red_weight, float green_weight, float blue_weight) :
    _red_weight(red_weight), _green_weight(green_weight), _blue_weight(blue_weight) {}
    ...
    for (unsigned int y = 0; y < img->size_y; y++)
    {
        for (unsigned int x = 0; x < img->size_x; x++)
        {
            ...
            case AAC_Pixel_Type::G:
            {
                ...
                brightness_matrix->GetElementReference(x, y) = pixels_g->GetElementReference(x, y).GetPixelValues().grey*(_red_weight + _green_weight + _blue_weight)/3;
                break;
            }
            case AAC_Pixel_Type::GA:
            {
                ...
                brightness_matrix->GetElementReference(x, y) = (ga.grey + ga.alpha)*(_red_weight + _green_weight + _blue_weight)/3 / 2;
                break;
            }
            case AAC_Pixel_Type::RGB:
            {
                ...
                brightness_matrix->GetElementReference(x, y) = rgb.red*_red_weight / 3 + rgb.green*_green_weight / 3 + rgb.blue*_blue_weight / 3;
                break;
            }
            case AAC_Pixel_Type::RGBA:
            {
                ...
                brightness_matrix->GetElementReference(x, y) = rgba.red*(_red_weight / 6) + rgba.green*(_green_weight / 6) + rgba.blue*(_blue_weight / 6) + rgba.alpha / 2;
                break;
            }
            default:
